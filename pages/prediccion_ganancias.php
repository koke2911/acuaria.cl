...
<head>
    <meta charset="utf-8" />
    <title>Predicción de Ganancias (Chart.js)</title>
    ...
    <style>
        ...
        * {
        ...
        body {
            ...
            background: radial-gradient(1200px 800px at 15% -10%, rgba(45, 212, 191, .12), transparent 60%),
...
<body>
    <div class="wrap">
        <div class="head">
            <div class="brand">
                <!-- <div class="logo"></div>
                <h1 id="title">Predicción de Ganancias CLP$</h1> -->
            </div>
        ...
        <div class="card panel" style="margin-bottom:12px;">
            <input id="apiBase" value="<?php echo $API_GANANCIAS; ?>" readonly type="hidden" />
    ...
    <script>
        ...
        function renderMetaChips(meta, opts) {
            // Título con APR nombre si viene
            $('#title').textContent = `Predicción de Ganancias CLP$${meta.apr_nombre ? ' — ' + meta.apr_nombre : ''}`;
                        ...
                        tooltip: {
                            callbacks: {
                                label: (ctx) => ctx.dataset.label === 'Pronóstico' ?
                                    `Pronóstico: ${fmtCLP(Math.round(ctx.parsed.y))}` : null,
                                afterBody: (items) => {
                                    ...
                                    return `Rango esperado: ${fmtCLP(Math.round(lo))} – ${fmtCLP(Math.round(up))}`;
                                }
                            }
                        }
                    },
